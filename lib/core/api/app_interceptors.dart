...
class AppInterceptors extends Interceptor {
  ...
  void onRequest(
    ...
    if (EndPoints.token.isNotEmpty) {
      options.headers
          .addAll({EndPoints.authorization: 'Bearer ${EndPoints.token}'});
    }
    options.headers[HttpHeaders.acceptHeader] = ContentType.json;
    String language = await _appPreferences.getAppLanguage();
    options.headers[EndPoints.apiKey] = EndPoints.keyForApiKey;
    options.headers[EndPoints.defaultLanguage] = language;
